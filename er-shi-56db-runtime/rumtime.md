...
## 4.网络数据 生成 字典
- 用处
> 当从 服务器上获取到数据，需要进行 字典-》模型
> * 此时 定义模型的时候，需要把字典中的 key 一个一个定义一遍，在key比较多的时候，很麻烦
### 4.1 能不能 自动根据 字典中的ekey 自动生成需要定义的字符串？
- 首先 打印 一下 字典中 数据都是什么类型
![](/assets/QQ20170402-084434.png)
